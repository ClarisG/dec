...
try {
    ...
    if ($user_data) {
        ...
        // Use session profile picture first, then database
...
<body class="bg-gray-50">
        ...
        <div class="p-6 border-b border-blue-400/30">
            <div class="flex items-center space-x-4">
                <div class="relative">
                    ...
                    $session_profile_pic = $_SESSION['profile_picture'] ?? '';
                    $profile_pic_path = "../uploads/profile_pictures/" . $session_profile_pic;
                    
                    if (!empty($session_profile_pic) && file_exists($profile_pic_path)): 
                    ?>
    ...
    <script>
        ...
        function updateProfilePictures(imageUrl) {
            ...
            if (sidebarImg && imageUrl) {
                sidebarImg.src = imageUrl + '?t=' + Date.now();
            }
            
            // Update header profile image
            const headerImg = document.getElementById('headerProfileImage');
            if (headerImg && imageUrl) {
                headerImg.src = imageUrl + '?t=' + Date.now();
            }
            
            // Update dropdown profile image
            const dropdownImg = document.getElementById('dropdownProfileImage');
            if (dropdownImg && imageUrl) {
                dropdownImg.src = imageUrl + '?t=' + Date.now();
            }
            
            // Also update the default images to real images if they exist
            const sidebarDefault = document.getElementById('sidebarDefaultImage');
            const headerDefault = document.getElementById('headerDefaultImage');
            const dropdownDefault = document.getElementById('dropdownDefaultImage');
            
            if (imageUrl && sidebarDefault) {
                sidebarDefault.outerHTML = `<img src="${imageUrl}?t=${Date.now()}" alt="Profile" class="w-12 h-12 rounded-full object-cover border-2 border-white shadow-sm" id="sidebarProfileImage">`;
            }
            if (imageUrl && headerDefault) {
                headerDefault.outerHTML = `<img src="${imageUrl}?t=${Date.now()}" alt="Profile" class="w-10 h-10 rounded-full object-cover border-2 border-white shadow-sm" id="headerProfileImage">`;
            }
            if (imageUrl && dropdownDefault) {
                dropdownDefault.outerHTML = `<img src="${imageUrl}?t=${Date.now()}" alt="Profile" class="w-10 h-10 rounded-full object-cover" id="dropdownProfileImage">`;
            }
        }
        
        // Listen for profile picture updates (for AJAX implementation)
        document.addEventListener('profilePictureUpdated', function(e) {
            if (e.detail && e.detail.imageUrl) {
                updateProfilePictures(e.detail.imageUrl);
            }
        });
        
        // Check if we just came from profile update
        if (sessionStorage.getItem('profileUpdated') === 'true') {
            ...
            setTimeout(() => {
                ...
                document.querySelectorAll('img[alt="Profile"]').forEach(img => {
                    if (img.src.includes('uploads/profile_pictures/')) {
